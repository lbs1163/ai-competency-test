...
export default function ShapeRotationPage() {
  ...
  const userTransform = operationsToCssTransform(steps);
  ...
  const finishRound = useCallback(() => {
    setSummaries((current) => {
      ...
      return [...filtered, { round, solved: roundSolved, optimal: roundOptimal }].sort((a, b) => a.round - b.round);
    });
    setSteps([]);
    setRemainingMs(0);
    if (round === 1) {
  ...
  }, [round, roundOptimal, roundSolved]);
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
          <section className="rounded-[28px] border border-stone-200 bg-white p-4 shadow-sm sm:p-6">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-[1fr_72px_1fr] md:items-center">
              <ShapePanel title="전" problem={problem} transform={userTransform} hint="현재 입력 결과" />
            ...
            <div className="rounded-2xl border border-stone-200 p-4 text-sm text-stone-600">
                ...
                {[1, 2].map((value) => {
                  ...
                  return (
                    <div key={value} className="flex items-center justify-between rounded-xl bg-stone-50 px-3 py-2">
                      <span>{value}라운드</span>
                      <span className="font-semibold text-stone-900">{summary ? `${summary.solved}개 / 최소 ${summary.optimal}개` : "대기 중"}</span>
                    </div>
                  );
                })}
              </div>
              {phase === "finished" && (
                <div className="mt-4 rounded-xl bg-emerald-50 p-3 font-semibold text-emerald-700">
                  <CheckCircle className="mr-1 inline" size={16} /> 총 정답 {totalSolved}개, 최소 클릭 해결 {totalOptimal}개
                </div>
              )}
            </div>
          </aside>
